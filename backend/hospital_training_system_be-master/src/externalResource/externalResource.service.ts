...
export class ExternalResourceService {
  ...
  async getExternalResourceByTopicId(topicId: number) {
    try {
      const externalResources = await this.prisma.externalResource.findMany({
        where: {
          isDeleted: false,
          topicId: topicId,
        },
      ...
      for (const externalResource of externalResources) {
        //Get Attachment of External Resource
        ...
        const externalResourceAndAttachments = {
          ...externalResource,
          attachment,
        };
        allExternalResources.push(externalResourceAndAttachments);
      }
  ...
  async getExternalResourceById(externalResourceId: number) {
    try {
      ...
      return externalResourceAndAttachments;
    } catch (error) {
      throw error;
    }
  }
