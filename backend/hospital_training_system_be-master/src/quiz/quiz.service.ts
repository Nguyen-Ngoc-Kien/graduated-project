...
export class QuizService {
  ...
  async getGradesOfQuiz(quizId: number) {
    const shuffledQuizzes = await this.prisma.quiz.findMany({
      where: {
    ...
    //Get quizAttempt
    for (const shuffledQuiz of shuffledQuizzes) {
      ...
      if (quizAttempt) {
        const trainee = await this.prisma.user.findUnique({
          where: {
            id: quizAttempt.userId,
          },
        });
        delete trainee.hash;
        delete trainee.hashedRt;
  
        const quizAttemptAndTrainee = { ...quizAttempt, trainee };
